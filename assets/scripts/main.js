...
function saveRecipesToStorage(recipes) {
  ...
  //            header. It is possible in only a single line, but should
  //            be no more than a few lines.
  // const string = JSON.stringify (recipes);
...
function initFormHandler() {
  ...
  const new_recipe = document.getElementById ("new-recipe");
  // B3. TODO - Add an event listener for the 'submit' event, which fires when the
  //            submit button is clicked
  new_recipe.addEventListener ("submit", () => {
  // Steps B4-B9 will occur inside the event listener from step B3
  // B4. TODO - Create a new FormData object from the <form> element reference above
    let data = new FormData (new_recipe);
  ...
  const clear_local_storage = document.getElementsByClassName ("danger")[0];
